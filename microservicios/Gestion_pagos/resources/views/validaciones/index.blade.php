...
<body>
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div id="voucher_data" class="grid-item"></div>
            </div>
        </div>
    </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Eliminar la fila del voucher validado en la tabla
                    $('#fila-voucher-' + pagosSiggaId).remove();
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
            ...
            error: function(xhr) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error al validar el voucher: ' + xhr.responseJSON.message,
                });
            }
        });
    });
    ...
    function refrescarTablaVouchers() {
        $.ajax({
            url: '/validaciones', // Ajusta esta ruta si es necesario
            method: 'GET',
            success: function(response) {
                $('#pagos_sigga_table tbody').html(response); // Aquí asumimos que `response` devuelve las filas de la tabla
            },
            error: function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error al refrescar la tabla de vouchers.',
                });
            }
        });
    }
